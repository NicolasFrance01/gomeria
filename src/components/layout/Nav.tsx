    ...
    return (
        <nav className="fixed bottom-0 w-full bg-surface-light/90 dark:bg-surface-dark/95 backdrop-blur-xl border-t border-slate-200 dark:border-slate-800 pb-safe pt-2 px-6 pb-2 z-50">
            <div className="max-w-md mx-auto flex justify-between items-end h-[60px]">
                ...
                <Link href="/stock" className="flex flex-col items-center gap-1 group w-16 pb-2">
                    <Package className={cn("w-6 h-6 transition-all duration-300", pathname === "/stock" ? "text-primary -translate-y-1" : "text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300")} strokeWidth={pathname === "/stock" ? 2.5 : 2} />
                    <span className={cn("text-[10px] font-medium transition-colors", pathname === "/stock" ? "text-primary" : "text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300")}>Stock</span>
                </Link>
